...
也可以将 `model` 替换为 `optimization_config_path="./configs/optimization.yaml"`，加载显式 OptimizationConfig。`apply()` 每个训练进程只能调用一次，不负责加载 RuntimeConfig 或启动训练；调用方需要在应用完成后再导入并执行模型代码。完整参数、返回值和异常行为见 [Python API 参考](docs/zh/reference/python_api.md)。

安装方式和环境检查见[安装指南](docs/zh/get_started/installation.md)。首次运行、完整启动参数、配置规则和优化应用日志说明见[快速开始](docs/zh/get_started/quick_start.md)、[CLI 参考](docs/zh/reference/cli.md)和[优化应用报告](docs/zh/user_guide/report.md)。

## 按角色导航

| 角色         | 典型场景                                         | 文档入口                                                                                                                                   |
| ------------ | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ |
| 训练用户     | 在训练任务中启用优化，确认应用状态并排查问题     | [安装指南](docs/zh/get_started/installation.md) → [快速开始](docs/zh/get_started/quick_start.md) → [模型支持清单](docs/zh/models/support_list.md) |
...
日常查询：[CLI 参考](docs/zh/reference/cli.md) · [Python API 参考](docs/zh/reference/python_api.md) · [优化应用报告](docs/zh/user_guide/report.md) · [问题排查](docs/zh/user_guide/troubleshooting.md) · [FAQ](docs/zh/faq.md)。完整目录见[文档中心](docs/README.md)。
