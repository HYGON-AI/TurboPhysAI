...
def _compile_model(model, target, mode):
    import torch
    ...
    if target == "camera":
        ...
        model.decoder["backbone"] = torch.compile(
            model.decoder["backbone"],
            mode=mode,
            fullgraph=False,
            dynamic=False,
        )
        model.decoder["neck"] = torch.compile(
            model.decoder["neck"],
            mode=mode,
            fullgraph=False,
            dynamic=False,
    ...
    elif target not in {"", "none", "off", "false", "0"}:
        raise ValueError(
            f"unknown TurboPhysAI BEVFusion compile target: {target}"
        )
    return model
...
def training_wrapper(original, options):
    ...
    def wrapped(model, dataset, cfg, *args, **kwargs):
        ...
        compile_target = str(
            options.get(
                "compile_target", os.getenv("MMDET3D_COMPILE_TARGET", "camera")
            )
        ).strip().lower()
        compile_mode = str(
            options.get(
                "compile_mode",
                os.getenv(
                    "MMDET3D_COMPILE_MODE", "max-autotune-no-cudagraphs"
                ),
            )
        )
        model = _compile_model(model, compile_target, compile_mode)
