...
def compile_wrapper(original, options):
    ...
    # Match ``@torch.compile(mode=...)`` in the optimized source exactly.
    # In particular, leaving ``dynamic`` unspecified preserves automatic
    # dynamic-shape promotion for variable-size point and sparse tensors.
