# TurboPhysAI 文档中心

本文档中心按使用角色组织。第一次接触 TurboPhysAI 时，先阅读“安装与快速开始”，不要从内部架构开始。

## 了解 TurboPhysAI

- [TurboPhysAI 概述](zh/overview.md)

## 安装与快速开始

- [安装指南](zh/get_started/installation.md)
- [快速开始](zh/get_started/quick_start.md)
- [模型支持清单](zh/models/support_list.md)
- [模型应用说明](../model_examples/README.md)

## 训练用户指南

- [OptimizationConfig 使用指南](zh/user_guide/optimization_config.md)
- [RuntimeConfig 使用指南](zh/user_guide/runtime_config.md)
- [优化应用报告](zh/user_guide/report.md)
- [问题排查](zh/user_guide/troubleshooting.md)
- [常见问题](zh/faq.md)

## 优化开发指南

- [优化开发与接入流程](zh/developer_guide/optimization_workflow.md)
- [定义优化与组织优化组](zh/developer_guide/optimization_declarations.md)
- [生成和检查 OptimizationConfig](zh/developer_guide/optimization_config_generation.md)
- [优化验证与交付](zh/developer_guide/validation.md)
- [自定义算子接入](zh/developer_guide/custom_operator.md)

## 设计与维护

- [组件架构](zh/design/architecture.md)
- [优化检查、执行与回滚](zh/design/execution.md)
- [兼容性管理](zh/design/compatibility.md)
- [CLI 参考](zh/reference/cli.md)
