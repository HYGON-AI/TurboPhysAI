...
调试时可统一关闭组件内的所有 `torch.compile` 路径：

```bash
turbo-physai run \
  --model <model-name> \
  --set TURBO_PHYSAI_DISABLE_TORCH_COMPILE=1 \
  python path/to/train.py
```

该变量仅跳过 `torch.compile` 构造，不改变 Optimization Group 的选择和应用报告。需要在正式配置中关闭特定编译区域时，使用 `--disable-group` 指定对应的 compile Group。
