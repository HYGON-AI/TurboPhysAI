...
OptimizationReport 日志默认关闭。需要确认优化应用详情或排查问题时，在启动命令中增加 `--log-report`：
...
该参数只控制报告日志，不影响优化检查和应用。启用后，Rank 0 输出完整报告，并使用固定标记界定报告范围：
...
同一次 `turbo-physai run` 启动的各进程使用相同 Run ID，可据此关联分布式训练日志。TurboPhysAI 不创建独立报告文件。
...
OptimizationConfig 和 RuntimeConfig 均记录本次实际加载的绝对路径。未使用 RuntimeConfig 时，日志显示 `not used`。Python API 返回的 `OptimizationReport` 对象还包含运行环境快照、全部 Group 检查、Replacement 成员结果、目标修改记录和回滚结果，可供程序化检查。
...
日志在 `Preparation` 中展开 `warning`、`fail` 和 `unknown` 的 Group 检查。Python API 返回的报告对象保存全部检查，包括 target、Replacement、aliases、签名、Hash 和 Group 兼容条件。
...
`skipped` 可以大于 0，表示部分 Group 不参与本次执行。常见原因包括配置中的 `enabled: false`、通过 `--disable-group` 临时禁用，以及所依赖的 Group 被禁用。具体原因见 `Preparation` 中对应 Group 的 `reason`；预期执行的 Group 应全部计入 `applied`。
...
确认某项优化证据变化后仍安全，可以在启动时使用--force-group 指定 Group放行：
