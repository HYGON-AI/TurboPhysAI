...
| `optimization.yaml` | `optimization generate` 生成 | 展开的 Group、参考 commit，以及目标的 source、AST 或原生扩展文件证据 |
...
`optimization.yaml` 保存生成时使用的模型 commit。`optimization check` 不因当前 HEAD 与该参考 commit 不同而失败；运行 `turbo-physai run` 或调用 `turbo_physai.apply()` 时，commit 匹配结果以 `project.commit` 检查项写入 OptimizationReport。目标代码证据不匹配仍会阻断相应 Group。
