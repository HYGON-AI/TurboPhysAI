...
本指南规定 Optimization Group 从开发完成到交付使用前需要完成的验证。验证目标包括优化声明可正确应用、计算结果符合基线、模型训练行为稳定，以及性能收益能够复现。
...
| 性能验证 | 声明性能收益的优化 | 在统一测试条件下获得可复现的稳态收益 |
| 专项验证 | 使用条件分发、编译、导入兼容或分布式能力的优化 | 对应能力的适用路径和边界均已覆盖 |

前五类验证按优化实际影响范围执行；专项验证仅在 Optimization Group 使用相应能力时执行。
...
声明性能收益或包含专项能力时，还必须满足：

- [ ] 性能与显存达到交付目标，测试口径和结果可复现；
