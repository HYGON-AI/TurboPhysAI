...
本文说明如何通过组件附带产品镜像，在官方 BEVFormer 仓库基线上应用 TurboPhysAI 的 HCU 优化。通用操作流程参见[快速开始](../../docs/zh/get_started/quick_start.md)。
...
镜像 `harbor.sourcefind.cn:5443/dcu/admin/base/turbophysai:2.7.1-ubuntu22.04-dtk26.04-py3.10` 已预装 TurboPhysAI、HCU 运行环境及 BEVFormer 训练依赖，并包含经过验证的 OptimizationConfig 和 RuntimeConfig。镜像不包含 BEVFormer 模型源码；用户需要准备官方模型仓库，推荐使用上文优化接入基线，并将模型仓库、数据集和训练输出目录挂载到容器中。容器内无需重新安装 TurboPhysAI 或模型依赖。
...
`MODEL_ROOT` 必须指向官方 BEVFormer 仓库，且当前 commit 必须为 `66b65f3a1f58caf0507cb2a971b9c0e7f842376c`。在已配置 HCU 容器运行环境的主机上，挂载模型、数据和工作目录启动镜像：
...
下文命令均在容器内的 `/workspace/model/BEVFormer` 执行。模型代码来自宿主机挂载的官方仓库，TurboPhysAI 和运行依赖由镜像提供。
