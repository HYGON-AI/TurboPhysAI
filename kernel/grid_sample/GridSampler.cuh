// Derived from PyTorch: aten/src/ATen/native/cuda/GridSampler.cuh
...
typedef float floatx4 __attribute__((vector_size(4 * sizeof(float))));
...
// NHWC 用：向 (h,w) 位置的 4 个连续 channel 做 float4 原子加
template<typename index_t>
__forceinline__ __device__
void safe_add_2d_float4(float *data, int h, int w,
                        index_t sH, index_t sW, int H, int W,
                        const index_t base_offset,
                        floatx4 delta,
                        const index_t memory_span) {
  if (within_bounds_2d(h, w, H, W)) {
    const index_t offset = base_offset + h * sH + w * sW;
    #ifdef __gfx936__
      floatx4* v4f32_ptr = reinterpret_cast<floatx4*>(data + offset);
      __builtin_amdgcn_global_atomic_fadd_v4f32(v4f32_ptr, delta);
    #else
      fastAtomicAdd(data, offset + 0, memory_span, delta[0], true);
      fastAtomicAdd(data, offset + 1, memory_span, delta[1], true);
      fastAtomicAdd(data, offset + 2, memory_span, delta[2], true);
      fastAtomicAdd(data, offset + 3, memory_span, delta[3], true);
    #endif
  }
}
...
void safe_add_2d(scalar_t *data, int h, int w,
                 ...
                 const index_t memory_span) {
  if (within_bounds_2d(h, w, H, W)) {
    fastAtomicAdd(data,  // input data
                  NC_offset + h * sH + w * sW, // grad_input offset: NC_offset + h * sH + w * sW
                  memory_span, // grad_input numel: N * C * H_in * W_in 没有使用
                  delta,
                  true);
  }
}
