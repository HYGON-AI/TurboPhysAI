...
inline bool SpconvEnvFlagEnabled(const char *name) {
...
inline void CanonicalizeIndicePairs(torch::Tensor indicePairs,
                                    torch::Tensor indiceNum,
                                    int64_t numActOut) {
  if (!SpconvEnvFlagEnabled("MMDET3D_SPCONV_CANONICAL_INDICE") ||
      numActOut <= 0 || indicePairs.numel() == 0) {
    return;
  }

  auto indicePairNumCpu = indiceNum.to({torch::kCPU});
  const int *indicePairNumPtr = indicePairNumCpu.data_ptr<int>();
  const int64_t kernelVolume = indicePairs.size(0);
  const int64_t numActIn = indicePairs.size(2);
  const int64_t keyStride = std::max<int64_t>(numActIn, numActOut) + 1;

  for (int64_t i = 0; i < kernelVolume; ++i) {
    const int64_t nHot = indicePairNumPtr[i];
    if (nHot <= 1) {
      continue;
    }

    auto pairForOffset = indicePairs.select(0, i);
    auto inIndex = pairForOffset.select(0, 0).narrow(0, 0, nHot);
    auto outIndex = pairForOffset.select(0, 1).narrow(0, 0, nHot);
    auto key = outIndex.to(torch::kInt64).mul_(keyStride).add_(
        inIndex.to(torch::kInt64));
    auto sortResult = key.sort(0, false);
    auto order = std::get<1>(sortResult);

    auto sortedIn = inIndex.index_select(0, order);
    auto sortedOut = outIndex.index_select(0, order);
    inIndex.copy_(sortedIn);
    outIndex.copy_(sortedOut);
  }
}
...
std::vector<torch::Tensor> getIndicePair(
  ...
  if (subM) {
    ...
    CanonicalizeIndicePairs(indicePairs, indiceNum, numActOut);
