...
std::string getenv_lower(const char* name, const char* default_value) {
  const char* value = std::getenv(name);
  std::string result = value == nullptr ? default_value : value;
  std::transform(result.begin(), result.end(), result.begin(),
                 [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
  return result;
}

bool use_legacy_sorted_voxelize_opt(const at::Tensor& points,
                                    const int max_points,
                                    const int max_voxels,
                                    const int NDim) {
  if (getenv_lower("MMDET3D_DISABLE_VOXELIZE_OPT", "0") == "1") {
    return false;
  }
  const std::string mode = getenv_lower("MMDET3D_VOXELIZE_OPT_MODE", "legacy");
  if (mode == "off" || mode == "0" || mode == "false" ||
      mode == "disable" || mode == "disabled") {
    return false;
  }
  return (mode == "legacy" || mode == "safe" || mode == "1" ||
          mode == "true" || mode == "on") &&
         points.scalar_type() == at::kFloat && points.dim() == 2 &&
         points.is_cuda() && max_points > 0 && max_voxels > 0 && NDim == 3;
}

int hard_voxelize_gpu_legacy_sorted(
    const at::Tensor& points, at::Tensor& voxels, at::Tensor& coors,
    at::Tensor& num_points_per_voxel, const std::vector<float> voxel_size,
    const std::vector<float> coors_range, const int max_points,
    const int max_voxels, const int NDim) {
  TORCH_CHECK(NDim == 3, "hard_voxelize_gpu_legacy_sorted only supports NDim=3");
  ...
  const int64_t grid_x = round((coors_x_max - coors_x_min) / voxel_x);
  const int64_t grid_y = round((coors_y_max - coors_y_min) / voxel_y);
  const int64_t grid_z = round((coors_z_max - coors_z_min) / voxel_z);

  auto float_options = points.options().dtype(points.scalar_type());
  auto long_options = points.options().dtype(at::kLong);
  auto voxel_size_t = at::tensor({voxel_x, voxel_y, voxel_z}, float_options);
  auto coors_range_t =
      at::tensor({coors_x_min, coors_y_min, coors_z_min}, float_options);

  auto cx =
      at::floor((points.select(1, 0) - coors_range_t.select(0, 0)) /
                voxel_size_t.select(0, 0))
          .to(at::kLong);
  auto cy =
      at::floor((points.select(1, 1) - coors_range_t.select(0, 1)) /
                voxel_size_t.select(0, 1))
          .to(at::kLong);
  auto cz =
      at::floor((points.select(1, 2) - coors_range_t.select(0, 2)) /
                voxel_size_t.select(0, 2))
          .to(at::kLong);

  auto valid = cx.ge(0)
                   .logical_and(cx.lt(grid_x))
                   .logical_and(cy.ge(0))
                   .logical_and(cy.lt(grid_y))
                   .logical_and(cz.ge(0))
                   .logical_and(cz.lt(grid_z));
  auto valid_idx = at::nonzero(valid).flatten();
  if (valid_idx.numel() == 0) {
    return 0;
  }

  auto keys = cx * (grid_y * grid_z) + cy * grid_z + cz;
  auto keys_valid = keys.index_select(0, valid_idx);
  auto order = at::argsort(keys_valid, true, 0, false);
  auto keys_sorted = keys_valid.index_select(0, order);
  auto idx_sorted = valid_idx.index_select(0, order);

  auto new_group = at::empty_like(keys_sorted, keys_sorted.options().dtype(at::kBool));
  new_group.index_put_({0}, true);
  if (keys_sorted.numel() > 1) {
    new_group.slice(0, 1)
        .copy_(keys_sorted.slice(0, 1).ne(
            keys_sorted.slice(0, 0, keys_sorted.size(0) - 1)));
  }

  auto group_starts = at::nonzero(new_group).flatten();
  const int64_t num_groups = group_starts.numel();
  if (num_groups == 0) {
    return 0;
  }

  auto group_ids = at::cumsum(new_group.to(at::kLong), 0) - 1;
  auto sorted_pos = at::arange(keys_sorted.numel(), long_options);
  auto rank_sorted = sorted_pos - group_starts.index_select(0, group_ids);

  auto end = at::full({1}, keys_sorted.numel(), long_options);
  auto group_ends = at::cat({group_starts.slice(0, 1), end}, 0);
  auto group_counts = group_ends - group_starts;

  auto first_idx_by_key = idx_sorted.index_select(0, group_starts);
  auto group_order = at::argsort(first_idx_by_key, true, 0, false);

  auto voxel_id_by_group = at::empty({num_groups}, long_options);
  voxel_id_by_group.index_put_({group_order}, at::arange(num_groups, long_options));
  auto voxel_id_sorted = voxel_id_by_group.index_select(0, group_ids);

  auto keep = voxel_id_sorted.lt(max_voxels).logical_and(rank_sorted.lt(max_points));
  if (keep.any().item<bool>()) {
    auto keep_idx = at::nonzero(keep).flatten();
    auto out_voxel_ids = voxel_id_sorted.index_select(0, keep_idx);
    auto out_ranks = rank_sorted.index_select(0, keep_idx);
    auto out_points = points.index_select(0, idx_sorted.index_select(0, keep_idx));
    voxels.index_put_({out_voxel_ids, out_ranks}, out_points);
  }

  const int64_t num_out = std::min<int64_t>(num_groups, max_voxels);
  auto selected_groups = group_order.slice(0, 0, num_out);
  auto first_points = idx_sorted.index_select(0, group_starts);
  auto coors_by_key = at::stack(
      {cx.index_select(0, first_points), cy.index_select(0, first_points),
       cz.index_select(0, first_points)},
      1);
  coors.slice(0, 0, num_out)
      .copy_(coors_by_key.index_select(0, selected_groups).to(coors.scalar_type()));

  auto counts = at::minimum(
      group_counts.index_select(0, selected_groups),
      at::full({num_out}, static_cast<int64_t>(max_points), long_options));
  num_points_per_voxel.slice(0, 0, num_out)
      .copy_(counts.to(num_points_per_voxel.scalar_type()));
...
int hard_voxelize_gpu(const at::Tensor& points, at::Tensor& voxels,
  ...
  if (use_legacy_sorted_voxelize_opt(points, max_points, max_voxels, NDim)) {
    return hard_voxelize_gpu_legacy_sorted(
