...
from test.msda_reference import multi_scale_deformable_attn_reference
...
def _install_mmcv_ext_stub(monkeypatch):
    """Expose the patched MMCV boundary without installing MMCV."""

    from turbo_physai.operators import multi_scale_deformable_attention

    mmcv = sys.modules.get("mmcv") or types.ModuleType("mmcv")
    ext = types.ModuleType("mmcv._ext")
    ext.ms_deform_attn_forward = (
        multi_scale_deformable_attention.ms_deform_attn_forward
    )
    ext.ms_deform_attn_backward = (
        multi_scale_deformable_attention.ms_deform_attn_backward
    )
    monkeypatch.setattr(mmcv, "_ext", ext, raising=False)
    monkeypatch.setitem(sys.modules, "mmcv", mmcv)
    monkeypatch.setitem(sys.modules, "mmcv._ext", ext)
    return ext
...
def test_bevformer_msda_compile_adapter_matches_reference(monkeypatch):
    _require_hcu()
    mmcv_ext = _install_mmcv_ext_stub(monkeypatch)
    from turbo_physai.optimizations.models.bevformer import msda

    monkeypatch.setattr(msda, "_mmcv_ext", mmcv_ext)
