...
def test_compiled_extract_img_feat_honors_disable_and_compile_mode(
    monkeypatch,
):
    ...
    monkeypatch.setenv("TURBO_PHYSAI_TORCH_COMPILE_MODE", "test-mode")
    monkeypatch.setattr(
        backbone.torch,
        "compile",
    ...
    assert calls == [(backbone.extract_img_feat, {"mode": "test-mode"})]
