...
class OptimizationConfigGeneratorTest(unittest.TestCase):
    ...
    def test_generate_can_omit_project_commit_for_public_config(self):
        public_template = textwrap.dedent(
            """
            metadata: {id: common.hcu.base, version: "1"}
            optimization_groups:
              - id: bevformer.msda
            """
        )
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            template = root / "template.yaml"
            template.write_text(public_template, encoding="utf-8")
            ...
            raw = yaml.safe_load(rendered)
            self.assertNotIn("commits", raw.get("compatibility", {}))
            self.assertNotIn("generation", raw)
            self.assertIn("trust", raw["optimization_groups"][0])
