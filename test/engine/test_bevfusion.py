...
BEVFUSION_GROUPS = (
    ...
    catalog.COMPILE_SPARSE_ENCODER,
    catalog.COMPILE_CAMERA,
    catalog.COMPILE_FUSER,
    catalog.COMPILE_DEPTH_TRANSFORM,
    catalog.COMPILE_TRANSFUSION,
)
...
class BevFusionPatchFrameworkTest(unittest.TestCase):
    ...
    def test_compile_catalog_covers_all_reference_decorators(self):
        ...
        self.assertEqual(
            {
                group.group_id: {spec.target for spec in group.specs}
            ...
            },
            {
                ...
                "bevfusion.compile.depth_transform": {
                    "mmdet3d.models.vtransforms.depth_lss.DepthLSSTransform"
                },
        ...
        compile_targets = {
            spec.target for group in compile_groups for spec in group.specs
        }
        self.assertEqual(
            compile_targets,
            {
        ...
        transfusion_compile = next(
            spec
            for spec in catalog.COMPILE_TRANSFUSION.specs
    ...
    def test_transfusion_group_covers_all_optimized_class_boundaries(self):
        ...
        compile_targets = {
            spec.target for spec in catalog.COMPILE_TRANSFUSION.specs
        }
        self.assertEqual(
            targets,
            {
    ...
    def test_transfusion_compile_wraps_migrated_forward(self):
        ...
        with mock.patch.dict(sys.modules, {"torch": fake_torch}):
            wrapped = compile_patches.compile_transfusion_forward_single_wrapper(
                lambda: "original", {"mode": "max-autotune-no-cudagraphs"}
            )
        ...
        self.assertEqual(
            calls[0][1], {"mode": "max-autotune-no-cudagraphs"}
        )
    ...
    def test_training_wrapper_passes_cfg_to_bev_metadata_prefetch(self):
        ...
        with mock.patch.dict(
            os.environ, {"TURBO_PHYSAI_DISABLE_TORCH_COMPILE": "1"}
        ), mock.patch.dict(sys.modules, {"torch": fake_torch}), mock.patch.object(
