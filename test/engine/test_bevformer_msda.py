...
BEVFORMER_GROUPS = (
    ...
    catalog.TSA,
    catalog.GRID_MASK,
    catalog.COMPILE_ENCODER,
    catalog.COMPILE_DECODER,
    catalog.TRAINING,
)
...
class BevFormerMsdaOptimizationEngineTest(unittest.TestCase):
    ...
    def test_catalog_import_is_runtime_free_and_default_registry_is_narrow(self):
        ...
        self.assertEqual(group.depends_on, ())
    ...
    def test_compile_targets_are_independently_selectable(self):
        self.assertEqual(
            tuple(spec.target for spec in catalog.COMPILE_ENCODER.specs),
            (
        ...
        self.assertEqual(
            tuple(spec.target for spec in catalog.COMPILE_DECODER.specs),
            (
    ...
    def test_missing_torch_custom_op_fails_when_replacement_is_resolved(self):
        ...
        fake_mmcv._ext = types.SimpleNamespace(
        ...
        try:
            with mock.patch.dict(
                sys.modules,
                {"torch": fake_torch, "mmcv": fake_mmcv},
            ):
    ...
    def test_real_torch_fake_mmcv_forward_backward_fake_and_compile(self):
...
msda._mmcv_ext.ms_deform_attn_forward = injected_failure
try:
    msda.MultiScaleDeformableAttnFunction_fp32.apply(
...
'''
    ...
    def test_second_spec_failure_rolls_back_entire_group(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            model_execution = next(
                item
                for item in report.execution
                if item.group_id == catalog.MSDA.group_id
            )
            self.assertEqual(model_execution.status.value, "rolled_back")
        self._assert_model_identity(modules, OriginalFp16, OriginalFp32)
    ...
    def test_complete_optimization_config_preserves_declared_group_order_and_trust(self):
        ...
        for entry in config.optimization_groups:
            ...
            for replacement_id in group.members:
                ...
                source_evidence = entry.trust["source_hashes"][target][0]
                if not source_evidence.startswith("artifact-v1:"):
        ...
        geometry = next(
            entry
            for entry in config.optimization_groups
            if entry.id == catalog.GEOMETRY.group_id
        )
