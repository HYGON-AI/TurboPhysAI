...
def multi_scale_deformable_attn_reference(
    ...
    split_sizes = [
        ...
        for height, width in value_spatial_shapes.detach().cpu().tolist()
    ]
    ...
    for level, (height, width) in enumerate(
        value_spatial_shapes.detach().cpu().tolist()
    ):
        level_value = (
            value_levels[level]
            .flatten(2)
    ...
    output = (
        torch.stack(sampled_levels, dim=-2)
        .flatten(-2)
        .mul(weights)
        .sum(-1)
        .view(batch, num_heads * channels, num_queries)
    )
